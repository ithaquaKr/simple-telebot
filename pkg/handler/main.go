...
const storyCommand string = "/story"

var lenStoryCommand int = len(storyCommand)

const startCommand string = "/start"

var lenStartCommand int = len(startCommand)

const botTag string = "@hi_am_cyno_bot"
...
const telegramTokenEnv string = "5772285972:AAGfog9OMQm1a0N0ePsEnlPKL8Rl8mRPcp4"
...
const jokeStoryApiEnv string = "https://api.api-ninjas.com/v1/jokes?limit=1"

var jokeStoryApi string = os.Getenv(jokeStoryApiEnv)
...
func HandleTelegramWebHook(w http.ResponseWriter, r *http.Request) {
	...
	var lyric, errRapLyrics = getJokesStory(sanitizedSeed)
	if errRapLyrics != nil {
...
// sanitize remove clutter like /start /story or the bot name from the string s passed as input
func sanitize(s string) string {
	...
	if len(s) >= lenStoryCommand {
		if s[:lenStoryCommand] == storyCommand {
			s = s[lenStoryCommand:]
		}
	}
	if len(s) >= lenBotTag {
...
// getPunchline calls the jokes API to get a joke story back.
func getJokesStory(seed string) (string, error) {
	jokeStoryResp, err := http.PostForm(
		jokeStoryApi,
		url.Values{"input": {seed}})
	if err != nil {
		log.Printf("error while calling jokes story %s", err.Error())
		return "", err
	}
	var joke Story
	if err := json.NewDecoder(jokeStoryResp.Body).Decode(&joke); err != nil {
		log.Printf("could not decode incoming punchline %s", err.Error())
		return "", err
	}
	defer jokeStoryResp.Body.Close()
	return joke.Jokes, nil
}
